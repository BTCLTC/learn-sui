...
module defi::pool {
    ...
    /// Fot when someone attempts to add more liquidity than u128 Math allows.
    const EPoolFull: u64 = 4;

    /// The interger scaling setting for fees calculation.
    const FEE_SCALING: u128 = 10000;

    /// The max value that can be held in one of the Balance of
    ...
    /// to create a type whic will mark LSPs.
    ...
    /// Entrypoint for the `swap_sui` method. Sends swapped token to sender.
    /// /// Swap `Coi<SUI>` for the `Coin<T>`.
    //     /// Returns Coin<T>.
    public entry fun swap_sui<P, T>(
        pool: &mut Pool<P, T>,
        sui: Coin<SUI>,
        ctx: &mut TxContext
    ) {
        ...
        let balance = coin::take(&mut pool.token, output_amount, ctx);
    ...
    /// to the sender.
    public entry fun swap_token<P, T>(
        pool: &mut Pool<P, T>,
        token: Coin<T>,
        ctx: &mut TxContext
    ) {
        ...
        let balance = coin::take(&mut pool.sui, output_amount, ctx);
        transfer::transfer(
            balance,
            tx_context::sender(ctx)
        )
    ...
    /// Entrypoint for the `add_liquidity` method. Sends `Coin<LSP>`
    /// to the transaction sender.
    public entry fun add_liquidity<P, T>(
        pool: &mut Pool<P, T>,
        sui: Coin<SUI>,
        token: Coin<T>,
        ctx: &mut TxContext
        ...
        let new_balance = coin::from_balance(balance, ctx);
    ...
    public entry fun remove_liquidity<P, T>(
        pool: &mut Pool<P, T>,
        lsp: Coin<LSP<P, T>>,
        ctx: &mut TxContext
    ) {
        ...
        let sui = coin::take(&mut pool.sui, sui_removed, ctx);
        let token = coin::take(&mut pool.token, tok_removed, ctx);
    ...
    public fun get_amounts<P, T>(pool: &Pool<P, T>): (u64, u64, u64) {
        (balance::value(&pool.sui), balance::value(&pool.token), balance::supply_value(&pool.lsp_supply))
    }
