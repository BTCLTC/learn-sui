module defi::shared_escrow {
    ...
    struct EscrowedObj<T: key + store, phantom U: key + store> has key, store {
        id: UID,
    ...
    public fun create<T: key + store, U: key + store>(
        ...
        transfer::share_object(EscrowedObj<T, U> {
            id,
            creator,
            recipient,
            exchange_for,
    ...
    /// the `receipient` of the escrow can exchange `obj` with the escrowed item
    public entry fun exchange<T: key + store, U: key + store>(
        obj: U,
        escrow: &mut EscrowedObj<T, U>,
        ctx: &mut TxContext
    ) {
    ...
    public entry fun cancel<T: key + store, U: key + store>(
        escrow: &mut EscrowedObj<T, U>,
        ctx: &mut TxContext
    ) {
