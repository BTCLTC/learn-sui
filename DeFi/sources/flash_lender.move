...
module defi::flash_lender {
    ...
    /// were borrwoed. Because this struct does not have the `key` or
    /// `store` ability, it cannot be transferred ot otherwise placed in
    ...
    struct Receipt<phantom T> {
        /// ID of the flash lender object the debt holder borrwoed from
        flash_lender_id: ID,
        /// Total amount of funds the borrower must repay: amount borrowed + fee
        repay_amount: u64
    }
    ...
    /// Tried to repay an amount other than `repay_amount` (i.e, the amount borrowed + fee)
    ...
    /// that the borrower will call `repay(lender, ...` later on it this tx.
    ...
    /// Return the amount that the holder of `slef` must repay
