...
module fungible_tokens::private_balance {
    ...
    /// Storabel balance - an inner struct of a PrivateCoin type.
    /// Can be used to store coins which don'g need to have the
    /// key ability.
    ...
    /// this blinding factor of `self` will be set to 0.
    ...
    public fun join<T>(self: &mut PrivateBalance<T>, ohter: PrivateBalance<T>) {
        ...
        if (option::is_some(&self.value) && option::is_some(&other.value)) {
            new_value = option::some(*option::borrow(&self.value) + *&other.value);
        };
    ...
    public fun split<T>(self: &mut PrivateBalance<T>, new_commitment: RistrettoPoint, proof: vector<u8>): PrivateBalance<T> {
        ...
        // In order to prevent new coins being minted, Open(A) - Open(B) must hold.
    ...
    /// Takes a public sub-balance from `slef`. Note that `self` retains its privacy option after this function.
    public fun split_to_public<T>(self: &mut PrivateBalance<T>, value: u64, proof: vector<u8>): PrivateBalance<T> {
        // TODO OPTIMIZATION:
        // 1. Add functionlity for Scalar to Point multiplication of the Pedersen Commitment base in FastCrypto.
        ...
        if (option::is_some(&self.value)) {
            ...
            slef.value = option::some(*option::borrow(&self.value) - value);
    ...
    #[test_only]
    /// Destory a `PrivacyBalance` with any value in it for testing purposes.
    public fun destory_for_testing<T>(self: PrivateBalance<T>): RistrettoPoint {
