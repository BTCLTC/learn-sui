module capy::capy {
    ...
    struct Genes has store, copy, drop {
        sequence: vector<u8>
    }
    ...
    fun init(ctx: &mut TxContext) {
        ...
        let capy_hash = sha3_256(object::uid_to_bytes(&id));

        emit(RegistryCreated {
            id: object::uid_to_inner(&id)
        });

        transfer::transfer(CapyManagerCap {
            id: object::new(ctx),
    ...
    public entry fun add_gene(
        ...
        emit(GeneDefinitionAdded {
            name,
            values
        });

        // lastly add new gene definition to the registry
        vec::push_back(&mut reg.genes, GeneDefinition {
            name,
            values
        });
    }
    ...
    public(friend) fun create_capy(
        reg: &mut CapyRegistry,
        sequence: vector<u8>,
        custom_attributes: vector<Attribute>,
        ctx: &mut TxContext
    ): Capy {
        let id = object::new(ctx);
        let genes = Genes {
    ...
    public entry fun add_item<T: key + store>(capy: &mut Capy, item: T) {
        ...
        dynamic_object_field::add(&mut capy.id, object::id(&item), item);
    }

    /// Remove item from the Capy.
    public entry fun remove_item<T: key + store>(capy: &mut Capy, item_id: ID) {
        emit(ItemRemoved<T> {
            capy_id: object::id(capy),
            item_id
        });
    ...
    fun img_url(uid: &UID): Url {
        let capy_url = IMAGE_URL;
        vec::append(&mut capy_url, sui::hex::encode(object::uid_to_bytes(uid)));
    ...
    fun test_raw_vec_to_values() {
        ...
        // expect [255, "blue"]
        {
            ...
            assert!(string::bytes(&name) == &b"blue", 1);
        };

        // expect [127, "red"]
        {
            let Value { selector, name } = vec::pop_back(&mut values);
            assert!(selector == 127, 3);
            assert!(string::bytes(&name) == &b"red", 4);
        };
